...
%         BleDeviceLedControl(9);
        ...
        % output header
        strtmp = sprintf('E(V)\tdI(uA)\tdI_s(uA)\tI1(LSB)\tI2(LSB)\tdI(LSB)');
        dlmwrite(swvfilename, strtmp, '-append', 'delimiter', '', 'newline', 'pc');
                
        % output data
        dlmwrite(swvfilename, [DataE' DataDeltaI' DataDeltaI_smooth' DataI1LSB' DataI2LSB' DataDeltaILSB'], ... 
...
%             RecvLongChar
        ...
        % output header
        strtmp = sprintf('Time(s)\tI(uA)\tI(LSB)\tI_s(uA)');
        dlmwrite(amperofilename, strtmp, '-append', 'delimiter', '', 'newline', 'pc');
        
        % output data
        dlmwrite(amperofilename, [TimeData' IData' AdcData' IDataFilt'], ... 
            '-append',  'delimiter', '\t', 'newline', 'pc');
    end
