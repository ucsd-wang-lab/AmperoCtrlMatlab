...
%%% T. Nakagawa
%%% 2016.?.?
%%% Updated 2019, C. Brown
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function swv_test_06_B
    clear all;
    close all;
...
%% parameters
    %ƒtƒ@ƒCƒ‹‚©‚ç“Ç‚Ý?ž‚Ý
    paramfile = '.\input\param_file_v03.dat';
...
%         end
        ...
        bool_stop = 0;
        starttime = datetime('now','TimeZone','local','Format','HH:mm:ss');
...
%         BleDeviceLedControl(9);
        ...
        DataDeltaI_smooth = smoothdata(DataDeltaI);
        %plot
        set(ax1,'xtick',[]);
        set(ax1,'ytick',[]);
        %plot(ax1, AdcData, '-x');
        %plot(ax1, DataE, DataDeltaILSB, '-x');
        plot(ax1, DataE, DataDeltaI_smooth, '-x');
        grid(ax1,'on');
        xlim auto;
        ylim auto;
        set(ax1,'FontName','Arial','FontSize',16);
        ...
        % output header
        strtmp = sprintf('E(V)\tI1(LSB)\tI2(LSB)\tdI(LSB)\tdI(uA)\tdI_s(uA)');
        dlmwrite(swvfilename, strtmp, '-append', 'delimiter', '', 'newline', 'pc');
                
        % output data
        dlmwrite(swvfilename, [DataE' DataI1LSB' DataI2LSB' DataDeltaILSB' DataDeltaI' DataDeltaI_smooth' ], ... 
...
%             RecvLongChar
        ...
        % output header
        strtmp = sprintf('Time(s)\tI(LSB)\tI(uA)\tI_avg(uA)');
        dlmwrite(amperofilename, strtmp, '-append', 'delimiter', '', 'newline', 'pc');
        
        % output data
        dlmwrite(amperofilename, [TimeData'  AdcData' IData' IDataFilt'], ... 
            '-append',  'delimiter', '\t', 'newline', 'pc');
    end
...
%% Led control
    function BleDeviceLedControl(LedOnOffValue)
        % clear serial buffer
        while (BleDongle.BytesAvailable > 0)
            fread(BleDongle,1);
        end        
