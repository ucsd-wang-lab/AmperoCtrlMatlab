...
%% figure
    ...
    bot_m = 0.1; % â∫ë§ó]îí 
    mid_m = 0.1; % ?„â∫ä‘ó]îí 
    ver_r = 0.38; % ?cï˚å¸äÑ?á
    col_r = 0.6; % â°ï˚å¸äÑ?á
    % button parameter
    buttonwidth = 150;
    buttonheight = 50;
    buttonposoffsetx = 50;
...
%% Parameter file read
    function filereadbutton_Callback(~,~)
        ...
        %General
%         dummy               = param(1);
        ComName             = char(param(2)); %COMÉ|?[Ég
%         BleDeviceID         = param(3);  
        deviceID            = char(param(4));    %ÉfÉoÉCÉXID
%         dummy               = param(5);
